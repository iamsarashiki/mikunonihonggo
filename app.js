...
// Database Progress (Offline)
let starredData = JSON.parse(localStorage.getItem('miku_stars')) || {};
let completedData = JSON.parse(localStorage.getItem('miku_completed')) || {};
let swiperInstance = null;
...
function selectChapter(num, element) {
    ...
    wrapper.innerHTML = ''; // Bersihkan container
    ...
    data.patterns.forEach(p => {
        ...
        // 2. Isi Data Teks
        slide.querySelector('.card-label').innerText = p.label;
        slide.querySelector('.card-desc').innerText = p.desc;
        slide.querySelector('.example-text').innerText = p.examples[0].jp;

        // 3. Set Status (Bintang & Check) dari Storage
        const starBtn = slide.querySelector('.star-btn');
        const checkBtn = slide.querySelector('.check-btn');

        if (starredData[p.id]) starBtn.classList.add('active');
        if (completedData[p.id]) {
            checkBtn.classList.add('active');
            slide.classList.add('is-completed');
        }

        // 4. Event Listener untuk Simpan Otomatis
        starBtn.onclick = (e) => {
            e.stopPropagation();
            starredData[p.id] = !starredData[p.id];
            if (!starredData[p.id]) delete starredData[p.id];
            starBtn.classList.toggle('active');
            localStorage.setItem('miku_stars', JSON.stringify(starredData));
        };

        checkBtn.onclick = (e) => {
            e.stopPropagation();
            completedData[p.id] = !completedData[p.id];
            if (!completedData[p.id]) delete completedData[p.id];
            checkBtn.classList.toggle('active');
            slide.classList.toggle('is-completed');
            localStorage.setItem('miku_completed', JSON.stringify(completedData));
        };

        slide.querySelector('.btn-learn-more').onclick = () => openDeepDive(p.id);

        // 5. Masukkan ke Slider
